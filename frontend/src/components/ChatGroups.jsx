...
export default function ChatGroups() {
  ...
  return (
    <div className="w-[50px] 2xs:w-[40px] xs:w-[45px] sm:w-40 md:w-50 p-2 2xs:p-1 xs:p-1.5 sm:p-3 border-r shrink-0 flex flex-col">
      <ul className="flex flex-col flex-1 overflow-y-auto gap-1.5 2xs:gap-0.5 xs:gap-1 sm:gap-1.5 md:gap-2">
        {chatGroups.map((group) => (
          ...
          >
            {group.name.length > 8 ? (
              <span className="text-xs 2xs:text-[10px] xs:text-sm sm:text-base">{group.name.substring(0, 8)}...</span>
            ) : (
